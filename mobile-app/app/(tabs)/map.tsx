...
function buildMapRegion(
  ...
  return {
    ...
    latitudeDelta: Math.max(0.04, (maxLatitude - minLatitude) * 1.45),
    longitudeDelta: Math.max(0.04, (maxLongitude - minLongitude) * 1.45),
  };
}
...
export default function MapTabScreen() {
  ...
  const loadMapData = useCallback(async () => {
    ...
    try {
      ...
      if (gpsSnapshot.status === "captured" && Number.isFinite(gpsLatitude) && Number.isFinite(gpsLongitude)) {
      ...
      if (cachedIncidents.length > 0) {
        setIncidents(cachedIncidents);
        setUsingOfflineCache(true);
        Alert.alert("Mapa", "Sin conexion actual. Mostrando la ultima cola sincronizada en el dispositivo.");
      } else {
  ...
  const mapRegion = useMemo(
    () => buildMapRegion(incidentsWithCoordinates, currentLocation, selectedIncidentId),
    ...
    <ScreenScaffold scroll={false} contentContainerStyle={styles.screenContent}>
      ...
      >
        <View style={styles.heroMetaRow}>
          ...
          <View style={[styles.heroMetaChip, { borderColor: palette.heroBorder, backgroundColor: palette.heroBg }]}>
            <Text style={[styles.heroMetaText, { color: palette.textPrimary }]}>
              {usingOfflineCache ? "Cache offline" : locationLabel}
            </Text>
          </View>
        </View>
      </ScreenHero>
      ...
      <View style={[styles.mapCard, { backgroundColor: palette.cardBg, borderColor: palette.cardBorder }]}>
        {incidentsWithCoordinates.length === 0 ? (
          <EmptyStateCard
            ...
            body="Las incidencias filtradas no tienen `target_lat/target_lng`. Igual podes abrir el detalle o navegar por direccion."
          />
        ) : (
          <MapView
            style={styles.map}
            ...
            {incidentsWithCoordinates.map((incident) => {
              ...
              return (
                <Marker
                  ...
                  description={incident.dispatch_address || incident.note || `Incidencia #${incident.id}`}
        ...
        >
          {selectedIncident ? (
            ...
            <EmptyStateCard
              ...
              body="Elegi un pin o cambia el filtro para ver una tarjeta de despacho."
            />
          )}
