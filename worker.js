...
function dashboardAssetSecurityHeaders() {
  // Dashboard assets are served as separate files; enforce strict CSP and anti-framing.
  return {
...
function buildDashboardFallbackHtml() {
  return `<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  ...
  <meta name="description" content="Dashboard de gesti�n de instalaciones de drivers">
...
<body>
  <main>
    ...
    <p>No se pudo cargar el binding ASSETS del dashboard. Revisa wrangler.toml.</p>
  </main>
</body>
</html>`;
}

async function maybeServeDashboardStaticAsset(request, env, routeParts, corsPolicy) {
  ...
  const securityHeaders = dashboardAssetSecurityHeaders();
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const dashboardStaticResponse = await maybeServeDashboardStaticAsset(
        request,
        env,
        routeParts,
        corsPolicy,
      );
      if (dashboardStaticResponse) {
        return dashboardStaticResponse;
      }

      // Compatibility fallback when static assets binding is unavailable.
      if (routeParts.length === 1 && routeParts[0] === "dashboard" && request.method === "GET") {
        const html = buildDashboardFallbackHtml();
...
`;
      ...
      if (routeParts.length === 1 && routeParts[0] === "manifest.json" && request.method === "GET") {
        const manifest = {
          ...
          "description": "Dashboard de gesti�n de instalaciones de drivers",
